...
test_type=["1 worker","2 workers","3 workers", "4 workers", "5 workers", "6 workers", "7 workers", "8 workers"]

y_size=len(test_type)

load_time = [0] * y_size
overall_time = [0] * y_size
shuffle_time = [0] * y_size
output_time = [0] * y_size
computing_time = [0] * y_size
jobs_time = [0] * y_size
...
for w in range(y_size):
	...
	file_ov = open("OverallTime"+cache+"/results_overall_"+ds_size+"/"+str(w+1)+"_worker_"+str(cores)+"_cores_overall_time.txt", "r") 
	count=0
	for line in file_ov:
		if line == "\n": break 
		overall_time[w] += float(line)
		count += 1    			
	overall_time[w] /= count
	file_ov.close()

	# average jobs_time between tests present in results_jobs_time directory
	file_ov = open("OverallTime"+cache+"/results_jobs_"+ds_size+"/"+str(w+1)+"_worker_"+str(cores)+"_cores_jobs_time.txt", "r") 
	count=0
	for line in file_ov:
		if line == "\n": break 
		jobs_time[w] += float(line)
		count += 1    			
	jobs_time[w] /= count
	file_ov.close()

	# average load_time between tests present in results_load_time directory
	file_ov = open("LoadTime"+cache+"/results_load_"+ds_size+"/"+str(w+1)+"_worker_"+str(cores)+"_cores_load_time.txt", "r") 
	count=0
	for line in file_ov:
		if line == "\n": break 			
		load_time[w] += float(line)		
		count += 1    			
	load_time[w] /= count
	...
	file_ov = open("OverallTime"+cache+"/results_shuffle_"+ds_size+"/"+str(w+1)+"_worker_"+str(cores)+"_cores_shuffle_time.txt", "r") 
	count=0
	for line in file_ov:
		if line == "\n": break 
		shuffle_time[w] += float(line)
		count += 1    			
	shuffle_time[w] /= count
	file_ov.close()

	# average output_time between tests present in results_output_time directory
	file_ov = open("OverallTime"+cache+"/results_output_"+ds_size+"/"+str(w+1)+"_worker_"+str(cores)+"_cores_output_time.txt", "r")	
	count=0
	for line in file_ov:
		if line == "\n": break 
		output_time[w] += float(line)
		count += 1    			
	output_time[w] /= count
	file_ov.close()
...
plt.xlabel('Seconds')
plt.ylabel('Y workers with '+ cores +' cores')
plt.show()
