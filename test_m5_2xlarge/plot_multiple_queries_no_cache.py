...
ax.plot(queries, average_overall_time, label='Overall Time', color='red')
ax.plot(queries, average_read_time, label='Read Time', color='green')
ax.plot(queries, average_stage0_time, label='Stage 0 Time', color='blue')
leg1 = ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
ax.add_artist(leg1)
ax.set_title("Multiple Queries no cache")
