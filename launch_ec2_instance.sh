...
if [[ ( $1 == "" || $2 == "") ]]; then
	...
	# before executing this script make sure that you have an AMI of the VM that you would like to clone and also create a template from the Web Console specifying the placement group and the instance type 
        aws ec2 run-instances   --image-id ami-id --key-name template-id --subnet-id subnet-id --launch-template LaunchTemplateName=$1 --count $2 && echo "$2 Instances with name $1 have been launched, check the web console."
fi
